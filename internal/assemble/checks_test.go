...
func TestPDeltaSofteningUsesMemberAxial(t *testing.T) {
	m := &model.Model{
		Nodes: []model.Node{
			{ID: "A", X: 0, Y: 0, Support: "fixed"},
			{ID: "B", X: 0, Y: 4},
		},
		Elements: []model.Element{{From: "A", To: "B", E: 2.1e11, A: 0.01, I: 8e-5}},
		Loads:    []model.NodeLoad{{Node: "B", FY: -2e6, FX: -100}},
	}
	ratio, err := PDeltaSoftening(m)
	if err != nil {
		t.Fatalf("pdelta: %v", err)
	}
	if ratio >= 1 {
		t.Fatalf("compression should pull combined trace below elastic, ratio=%v", ratio)
	}
}
