...
func TestGeometricStiffnessSoftensCompression(t *testing.T) {
	g := &Geometry{Length: 4, C: 1, S: 0}
	E, I := 2.1e11, 8e-5
	ratio, err := SoftensUnderCompression(g, E, I, 5e5)
	if err != nil {
		t.Fatal(err)
	}
	if ratio >= 1 {
		t.Fatalf("compression should soften lateral stiffness, ratio=%v", ratio)
	}
	up, err := TensionStiffens(g, E, I, 5e5)
	if err != nil {
		t.Fatal(err)
	}
	if up <= 1 {
		t.Fatalf("tension should stiffen, ratio=%v", up)
	}
	kg, err := GeometricStiffness(g, -1e5)
	if err != nil {
		t.Fatal(err)
	}
	if kg[1][1] >= 0 {
		t.Fatalf("compressive kg[1][1] should be negative, got %v", kg[1][1])
	}
}
